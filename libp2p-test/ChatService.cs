...
public class ChatService
{
    ...
    private void OnMessageReceived(byte[] msg)
    {
        try
        {
            // Use source-generated context for AOT-friendly deserialization
            var json = Encoding.UTF8.GetString(msg);
            var chatMsg = JsonSerializer.Deserialize(json, ChatMessageJsonContext.Default.ChatMessage);
            if (chatMsg is not null)
            {
                lock (_messages)
                {
                    _messages.Add($"{chatMsg.SenderNick}: {chatMsg.Message}");
                    Console.WriteLine($"{chatMsg.SenderNick}: {chatMsg.Message}");
                }
            }
        }
        catch
    ...
    public void Publish(string message, string nickName)
    {
        ...
        // Use source-generated context for AOT-friendly serialization
        var payload = JsonSerializer.Serialize(chatMsg, ChatMessageJsonContext.Default.ChatMessage);
        _topic.Publish(Encoding.UTF8.GetBytes(payload));
